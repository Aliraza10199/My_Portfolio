...
(function ($) {
    ...
    $.fn.tablemanager = function(options = null) {
        ...
        **/
        ...
        arrayOptions = $.map(options, function(value, index){ return [index]; });
        for (i = 0; i < arrayOptions.length; i++){
            // check if options are in available options array
            if ( availableOptions.indexOf(arrayOptions[i]) === -1 ) { if (debug) { cLog('Error! '+arrayOptions[i]+' is unavailable option.'); } }
        }    
        ...
        **/
        if (options !== null) {
            ...
            **/
            if (options.vocabulary != '' && typeof options.vocabulary !== undefined && options.vocabulary !== undefined) {
                ...
                voc_filter_by = 
                    (options.vocabulary.voc_filter_by != '' && options.vocabulary.voc_filter_by !== undefined) ? 
                    options.vocabulary.voc_filter_by : voc_filter_by;

                voc_type_here_filter = 
                    (options.vocabulary.voc_type_here_filter != '' && options.vocabulary.voc_type_here_filter !== undefined) ? 
                    options.vocabulary.voc_type_here_filter : voc_type_here_filter;

                voc_show_rows = 
                    (options.vocabulary.voc_show_rows != '' && options.vocabulary.voc_show_rows !== undefined) ? 
                    options.vocabulary.voc_show_rows : voc_show_rows;
            ...
            **/
            if (options.disable != '' && typeof options.disable !== undefined && options.disable !== undefined) {
                for (var i = 0; i < options.disable.length; i++) {
                    ...
                    Heads.eq(col - 1).addClass('disableSort').removeClass('sortingAsc').removeClass('sortingDesc');
                    
                    // debug
                    if (isNaN(col - 1)) { if (debug) { cLog('Error! Check your "disable" option.'); } }
                }
            }
            ...
            **/
            ...
            if (options.showrows != '' && typeof options.showrows !== undefined && options.showrows !== undefined) {
                ...
                // div num rows
                var numrowsDiv = '<div  class="col-md-4 col-md-push-1" id="for_numrows" class="for_numrows" style="display: inline;z-index: 1;"><label for="numrows">'+translate(voc_show_rows)+': </label><select id="numrows"></select></div>';
                ...
                for (i = 0; i < showrows.length; i++) {
                    ...
                    // debug
                    if (isNaN(showrows[i])) { if (debug) { cLog('Error! One of your "show rows" options is not a number.'); } }
                }
            ...
            **/
            if (options.disableFilterBy != '' && typeof options.disableFilterBy !== undefined && options.disableFilterBy !== undefined) {
                for (var i = 0; i < options.disableFilterBy.length; i++) {
                    ...
                    // debug
                    if (isNaN(col - 1)) { if (debug) { cLog('Error! Check your "disableFilterBy" option.'); } }
                }
            }
            ...
            **/
            if(options.appendFilterby === true || Table.hasClass('tableFilterBy')) {
                ...
                var filterbyDiv = '<div class="col-md-4" id="for_filter_by" class="for_filter_by" style="display: inline;"><label for="filter_by">'+translate(voc_filter_by)+': </label><select id="filter_by"></select> <input id="filter_input" type="text" placeholder="'+translate(voc_type_here_filter)+'"></div>';
        ...
        if (debug) { cLog('Options set:', options); }
        ...
        **/
        function TableSort(rules) {
            ...
            for(i = 0; i < rlen; i++){
                ...
                cellsArray[i] = [];
                for(j = 0; j < clen; j++){
                    cellsArray[i].push(cells[j].innerHTML);
                }
            }
            ...
            for (i = 0; i < rules.length; i++) {
                ...
                var order = (rules[i][1] == undefined) ? 0 : ((rules[i][1] == 0 || rules[i][1] == 'asc') ? 0 : ((rules[i][1] == 1 || rules[i][1] == 'desc') ? 1 : 0));
        ...
        function multipleSortCol(array,data) {
            ...
            array.sort(function(a,b){
                for (i = 0; i < cols.length; i++) {
                    ...
                    if (i == cols.length && (a[cols[i]] == b[cols[i]])) { return 0; }
                    // check col type if is aplhpanum or date
                    if (types[i] == 'alphanumeric') {
                        if ((a[cols[i]] > b[cols[i]])) {
                            return order;
                        } else if ((a[cols[i]] < b[cols[i]])) {
                            return -1*order;
                        }
                    } else if (types[i] == 'date') {
                        if (formatDate(formats[i],a[cols[i]]) > formatDate(formats[i],b[cols[i]])) {
                            return order;
                        } else if (formatDate(formats[i],a[cols[i]]) < formatDate(formats[i],b[cols[i]])) {
                            return -1*order;
                        }
                    }
        ...
        **/
        function cLog(name,string = null) {
            console.log(name);
            if(string != null) {console.log(JSON.parse(JSON.stringify(string)));}
        };
    };
})(jQuery);